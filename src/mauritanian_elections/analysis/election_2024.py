"""Analysis module for 2024 Mauritanian presidential election."""
import sys
...
def run_2024_analysis():
    """Run complete analysis for 2024 election data."""
    print("Loading 2024 election data...")
    data = load_election_data(config.ELECTION_2024_CSV, sep=",")
    ...
    print("\nPreprocessing data...")
    election = preprocess_2024_data(data)
    ...
    # Create bar chart
    bar_chart_path = config.HTML_DIR / "2024_votes_by_wilaya.html"
    ...
    static_bar_path = config.FIGURES_DIR / "2024_votes_by_wilaya.png"
    print(f"Creating static bar chart: {static_bar_path}")
    create_static_bar_chart(grouped_cand, title="2024 Votes by Candidate and Wilaya", 
                           output_path=str(static_bar_path))
    
    # Create pie chart
    pie_chart_path = config.HTML_DIR / "2024_vote_distribution.html"
    ...
    static_pie_path = config.FIGURES_DIR / "2024_vote_distribution.png"
    print(f"Creating static pie chart: {static_pie_path}")
    create_static_pie_chart(grouped_cand, title="2024 Vote Distribution", 
                           output_path=str(static_pie_path))
    
    print("\n✓ 2024 analysis complete!")
